...
def main():
    ...
    MyFeed = namedtuple("MyFeed", "nf nf_url")

    f1 = MyFeed(nf="BBC News UK", nf_url="http://feeds.bbci.co.uk/news/uk/rss.xml")
    f2 = MyFeed(nf="BBC News Technology", nf_url="http://feeds.bbci.co.uk/news/technology/rss.xml")
    f3 = MyFeed(nf="Reuters UK", nf_url="http://feeds.reuters.com/reuters/UKdomesticNews?format=xml")
    f4 = MyFeed(nf="Reuters Technology", nf_url="http://feeds.reuters.com/reuters/technologyNews?format=xml")
    ...
    if conn is not None:
        with conn:
            ...
            my_feed_1 = (nf_title[0], nf_description[0], nf_link[0])
            nf_id_1 = dbf.create_feed(conn, my_feed_1)
            # create feeds for our feed entry
            for i in range(len(feeds_titles)):
                my_feed = (feeds_titles[i], feeds_descriptions[i], feeds_links[i], feeds_pubdate[i], nf_id_1)
                dbf.create_nf_entry(conn, my_feed)
